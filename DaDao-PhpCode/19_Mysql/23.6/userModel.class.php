...
$user_name=$username;
$result->execute();
$result->bind_result($user_name);//绑定结果
if ($result->fetch()) {
	$result->close();
    $mysqli->close();
	return FALSE;
...
$result->bind_param("ssssss",$user_name,$user_psw,$user_sex,$user_email,$user_phone,$user_address); //参数绑定                                                                    
$user_name=$registerArray['user_name'];
$user_psw=$registerArray['user_password'];
...
$user_psw=$userpsw;
$result->execute();
$result->bind_result($user_id,$user_name,$user_psw,$user_sex,$user_email,$user_phone,$user_address);//绑定结果
while ($result->fetch()) {
	$userList=array(
	'user_id'=>$user_id,
	'user_name'=>$user_name,
...
while ($result->fetch()) {
	...
	$userArray[]=$userList;//将保存不同用户信息的数组分别放入另一个数组中
}	
$result->close();
$mysqli->close();
