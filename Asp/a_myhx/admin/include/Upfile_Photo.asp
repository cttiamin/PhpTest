...
const UpFileType="jpg|gif|png|swf|rar"
	...
	for each formName in upload.file '列出所有上传了的文件
		...
		if FoundErr<>true then
			...
			if PhotoUrlID=0 then
					strJS=strJS & "parent.document.editform.previewimg.value='" & fileName & "';" & vbcrlf
					'strJS=strJS & "parent.document.myform.PhotoSize_Thumb.value='" & cstr(round(oFileSize/1024)) & "';" & vbcrlf
			end if
			if PhotoUrlID=1 then
					strJS=strJS & "parent.document.myform.Team_texPic.value='" & fileName & "';" & vbcrlf
					'strJS=strJS & "parent.document.myform.PhotoSize_Thumb.value='" & cstr(round(oFileSize/1024)) & "';" & vbcrlf
			end if
			if PhotoUrlID=2 then
					strJS=strJS & "parent.document.form01.PicUrl2.value='" & fileName & "';" & vbcrlf
					'strJS=strJS & "parent.document.myform.PhotoSize_Thumb.value='" & cstr(round(oFileSize/1024)) & "';" & vbcrlf
			end if
			if PhotoUrlID=3 then
					strJS=strJS & "parent.document.form01.PicUrl3.value='" & fileName & "';" & vbcrlf
					'strJS=strJS & "parent.document.myform.PhotoSize_Thumb.value='" & cstr(round(oFileSize/1024)) & "';" & vbcrlf
			end if
		else
